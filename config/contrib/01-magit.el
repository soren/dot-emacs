...
(let ((git (locate-file "git" exec-path exec-suffixes 'file-executable-p)))
  (if git
      (progn
        ...
        (if (eq system-type 'windows-nt)
            (progn
              ...
              (add-to-list 'exec-path
                           (replace-regexp-in-string "/cmd/" "/usr/bin" (file-name-directory git)))
              (slu-dot-emacs-clone-and-add "https://github.com/magit/ssh-agency")
              (require 'ssh-agency)))
